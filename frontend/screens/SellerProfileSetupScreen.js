...
export default function SellerProfileSetupScreen({ navigation }) {
  ...
  const pickImage = async (type) => {
    try {
      ...
      if (status !== 'granted') {
        showCustomAlert(
          "Permission Required", 
          "Permission to access gallery is required to choose profile and shop photos!", 
          [{ text: "OK", onPress: () => {} }], 
          "warning"
        );
        return;
  ...
  const handleComplete = async () => {
    if (!profileImage) {
      showCustomAlert(
        "Photo Required", 
        "Please upload a front-facing profile picture before finishing.", 
        [{ text: "OK", onPress: () => {} }], 
        "warning"
      );
      return;
    ...
    if (!fullName.trim() || !cnic.trim() || !occupation.trim()) {
      showCustomAlert(
        "Required Fields", 
        "Full Name, CNIC, and Occupation are required to complete your seller profile.", 
